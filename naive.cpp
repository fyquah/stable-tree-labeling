...
public:
  ...
  List(T hd, ListIntf<T>* tl) : m_hd(hd), m_tl(tl) {}
};


template<typename T>
using listptr = ListIntf<T>*;


template<typename T>
listptr<T> append(const T elem, const listptr<T> l) {
  return new List<T>(elem, l);
}
...
using node_id_t = std::vector<int> ;
...
typedef tree_t* treeptr;
typedef node_id_t* idptr;

static idptr make_sym(int i)
{
  auto ret = new node_id_t();
  ret->push_back(i);
  return ret;
}


std::vector<tree_t*> merge(
    const tree_t* declaration,
    const tree_t* application)
{
  std::vector<treeptr> children;

  for (const auto child : declaration->children) {
    auto new_id = new node_id_t(*application->id);
    new_id->insert(new_id->end(), child->id->begin(), child->id->end());

    auto new_child = new tree_t(child->label, new_id, child->children);
...
treeptr construct_tree(std::istream & fin)
{
  ...
  std::function<treeptr(int)> construct = [&](const int k){
    ...
    return new tree_t(ORIGINAL, make_sym(k), children);
  };
